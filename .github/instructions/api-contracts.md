...
Scope
- The user‑facing application communicates primarily via LiveStore’s sync protocol over WebSocket to a Cloudflare Durable Object. There is no traditional REST API in this studio. If you add HTTP APIs, define them separately.

Sync endpoint
- URL: `VITE_LIVESTORE_SYNC_URL` (dev: `http://localhost:8787`), configured in `studio/package.json` and consumed by `studio/src/livestore.worker.ts`.
- Protocol: WebSocket speaking LiveStore’s batched push/pull protocol handled by `@livestore/sync-cf`.

Authentication / validation
- The Cloudflare worker validates an auth payload via `validatePayload` in `studio/src/cf-worker/index.ts`.
- Dev default: expects `{ authToken: 'insecure-token-change-me' }` in the client `syncPayload` (set in `studio/src/Root.tsx`). Change this before production and load secrets via Cloudflare.
...
- The worker and client should remain compatible within a release line of `@livestore/*` packages pinned in `studio/package.json`.
