...
- Client worker: `studio/src/livestore.worker.ts` using `@livestore/adapter-web` and `@livestore/sync-cf`.
- Server: `studio/src/cf-worker/index.ts` via `makeDurableObject` and `makeWorker` from `@livestore/sync-cf/cf-worker`.
...
- Client provides `syncPayload` (e.g., `{ authToken }`) via `LiveStoreProvider` in `studio/src/Root.tsx`.
- Server validates with `validatePayload(payload)`; the studio requires `authToken === 'insecure-token-change-me'` (change in production).
...
- In the studio, these log to the console for visibility; extend to persist to D1 or run server‑side validations.

Persistence and backup
- D1 binding `DB` is configured in `studio/wrangler.toml`. Use it to persist server‑side indexes or backups of ops.
