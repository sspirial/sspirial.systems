# LiveStore Local-First studio

This repository contains a **LiveStore local-first application studio**. It demonstrates how to build a React + LiveStore app with:
...
## What this studio includes

Key files (under `studio/`):
- `studio/src/Root.tsx` – LiveStore provider setup (adapter, schema, sync payload, storeId)
- `studio/src/livestore.worker.ts` – Worker initialization + sync configuration
- `studio/src/livestore/schema.ts` – Tables, events (`v1.*`), and materializers
- `studio/src/livestore/queries.ts` – Sample query/selectors
- `studio/src/util/store-id.ts` – Stable per-replica identity via `?storeId=` query param
- `studio/src/cf-worker/index.ts` – Cloudflare Durable Object / Worker sync endpoint with payload validation
- `studio/vite.config.ts` – Vite config + Wrangler dev spawn + DevTools plugin
- `studio/wrangler.toml` – Durable Object and D1 bindings
- `studio/package.json` – Pinned LiveStore + React dependencies and dev scripts
...
- Prompt studios (bug fix, code generation, code review, migration, security, tests)
- Copilot instructions that define expected behaviors for automated assistance

## Using this studio for a new project "X"

When you create a new LiveStore project from this studio, you should:
1. Choose a project name: `X`
2. Instruct AI (e.g., Copilot/assistant) to perform a **rename from `studio` to `X`** across relevant files.
3. Update metadata and identifiers as described below.

### Minimum rename/actions list
Ask AI to:
- Rename folder `studio/` to `<X>/` (or move its contents to project src structure).
- Update `name` field in `studio/package.json` to something like `livestore-<x>`.
- Adjust event names if version scope changes (keep `v1.*` unless starting new version lineage).
- Replace any placeholder secrets (e.g., auth token in `studio/src/Root.tsx` and validation in `studio/src/cf-worker/index.ts`).
- Update `wrangler.toml` values (`name`, `database_name`, optional `database_id`).
- Search for `studio/src/livestore/schema.ts` references and ensure paths reflect the new location.
- Update documentation references in `.github/instructions/*` and `.github/prompts/*` from `studio/` to your new path.

### Suggested AI instruction example
```
Please rename the LiveStore studio to project "X":
1. Move/rename `studio/` directory to `x/`.
...
5. Update all docs/prompts referencing `studio/` to `x/`.
...
## Package manager
This studio uses Bun as the package manager.
