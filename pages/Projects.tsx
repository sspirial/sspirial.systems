...
const Projects: React.FC = () => {
  ...
  return (
    <main className="flex-1">
      ...
      <section className="sticky top-[72px] z-40 border-b border-[#e5e7eb] dark:border-gray-800 bg-background-light/95 dark:bg-background-dark/95 backdrop-blur">
          ...
          <div className="flex flex-wrap gap-2 overflow-x-auto pb-1 sm:pb-0 no-scrollbar">
            {filters.map((f) => (
              <button 
                key={f} 
                onClick={() => setFilter(f)}
                className={`group flex h-9 shrink-0 items-center justify-center gap-x-2 rounded-full px-4 transition-all ${
                  filter === f 
                  ? 'bg-primary text-white' 
                  : 'border border-[#e5e7eb] dark:border-gray-700 bg-white dark:bg-gray-800 text-slate-600 dark:text-slate-300 hover:border-primary/50'
                }`}
              >
      ...
      <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
          {filteredProjects.map((project) => (
            <article key={project.id} className="group flex flex-col overflow-hidden rounded-xl border border-[#e5e7eb] dark:border-gray-700 bg-white dark:bg-gray-900 transition-all hover:border-primary hover:shadow-lg dark:hover:border-primary">
              ...
              <div className="flex flex-1 flex-col p-5">
                ...
                <div className="mt-auto flex items-end justify-between border-t border-slate-100 dark:border-gray-800 pt-4">
                  <div className="flex flex-wrap gap-1">
                    {project.tags.map(tag => (
                      <span key={tag} className="rounded bg-slate-100 dark:bg-gray-800 px-2 py-1 text-[10px] font-semibold text-slate-600 dark:text-slate-400">{tag}</span>
                    ))}
                  </div>
                  <a className="group/btn flex items-center gap-1 text-sm font-bold text-primary hover:text-blue-700 dark:hover:text-blue-400" href="#">
                    Log
      ...
      <section className="mx-auto mt-8 max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <div className="flex flex-col items-center justify-center rounded-2xl bg-slate-900 dark:bg-primary/10 px-6 py-16 text-center shadow-inner">
          ...
          <div className="flex flex-col gap-4 sm:flex-row">
            <button className="flex h-12 items-center justify-center rounded-lg bg-primary px-8 text-base font-bold text-white transition-colors hover:bg-blue-600">Start Collaboration</button>
            <Link to="/about" className="flex h-12 items-center justify-center rounded-lg border border-slate-600 bg-transparent px-8 text-base font-bold text-white transition-colors hover:bg-slate-800 dark:hover:bg-gray-800">Read About Us</Link>
          </div>
        </div>
      </section>
    </main>
