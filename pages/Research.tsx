...
const Research: React.FC = () => {
  const featured = RESEARCH_POSTS.find(p => p.featured);
  const others = RESEARCH_POSTS.filter(p => !p.featured);
    ...
    <main className="flex-grow w-full max-w-7xl mx-auto px-4 md:px-10 py-8">
      ...
      {featured && (
        <section className="mb-16">
          <div className="group relative overflow-hidden rounded-xl bg-white dark:bg-surface-dark border border-gray-200 dark:border-gray-800 shadow-sm hover:shadow-md hover:border-primary/30 transition-all duration-300">
      ...
      <div className="sticky top-[72px] z-40 bg-background-light/95 dark:bg-background-dark/95 backdrop-blur-sm py-4 mb-8 -mx-4 px-4 md:-mx-10 md:px-10 border-b border-gray-200/50 dark:border-gray-800/50">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex overflow-x-auto pb-1 md:pb-0 no-scrollbar gap-6 md:gap-8 min-w-0 border-b md:border-b-0 border-gray-200 dark:border-gray-800">
            <a className="whitespace-nowrap flex flex-col items-center justify-center border-b-2 border-primary text-primary pb-2" href="#">
              <p className="text-sm font-bold leading-normal tracking-[0.015em]">All Posts</p>
            </a>
            {['Research', 'Dev Logs', 'Whitepapers'].map(tab => (
              <a key={tab} className="whitespace-nowrap flex flex-col items-center justify-center border-b-2 border-transparent hover:border-gray-300 dark:hover:border-gray-600 text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 transition-colors pb-2" href="#">
          ...
          <div className="w-full md:w-auto md:min-w-[320px]">
            <div className="relative group">
              ...
              <input className="block w-full rounded-lg border-0 py-2.5 pl-10 pr-4 text-slate-900 ring-1 ring-inset ring-gray-200 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-primary sm:text-sm sm:leading-6 bg-white dark:bg-surface-dark dark:ring-gray-700 dark:text-white dark:placeholder:text-gray-500 transition-shadow shadow-sm" placeholder="Search documentation..." type="text"/>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {others.map((post) => (
          <article key={post.id} className="flex flex-col bg-white dark:bg-surface-dark rounded-xl p-6 border border-gray-200 dark:border-gray-800 hover:border-primary/40 dark:hover:border-primary/40 transition-all hover:shadow-lg hover:shadow-gray-100 dark:hover:shadow-none group h-full">
            ...
            <div className="mt-auto pt-4 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between">
              <div className="flex gap-2">
                {post.tags.map(tag => (
                  <span key={tag} className="text-[10px] font-semibold text-slate-400 bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 rounded">{tag}</span>
                ))}
              </div>
