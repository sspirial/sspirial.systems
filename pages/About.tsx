    ...
    <div className="layout-container flex h-full grow flex-col">
      <div className="px-5 md:px-40 flex flex-1 justify-center py-5">
        <div className="layout-content-container flex flex-col max-w-[960px] flex-1">
          ...
          <section className="flex flex-col gap-6 py-10 lg:flex-row items-center">
            ...
            <div className="flex flex-col gap-6 lg:justify-center lg:w-1/2 lg:pl-10">
              ...
              <div className="flex gap-4">
                <button className="flex min-w-[84px] cursor-pointer items-center justify-center rounded-lg h-12 px-6 bg-primary text-white text-base font-bold shadow-lg hover:bg-blue-700 transition-all">Read Manifesto</button>
                <button className="flex min-w-[84px] cursor-pointer items-center justify-center rounded-lg h-12 px-6 bg-white dark:bg-slate-800 border border-[#dbdfe6] dark:border-gray-700 text-[#111318] dark:text-white text-base font-bold hover:bg-gray-50 dark:hover:bg-gray-700 transition-all">View Research</button>
              </div>
            </div>
          </section>

          {/* Philosophy Section */}
          <section className="flex flex-col gap-10 py-16 border-t border-[#f0f2f4] dark:border-gray-800">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                ...
                <div key={idx} className="flex flex-1 gap-4 rounded-xl border border-[#dbdfe6] dark:border-gray-700 bg-white dark:bg-slate-800 p-6 flex-col hover:border-primary/50 transition-colors group">
          ...
          {/* Evolution / Timeline */}
          <section className="py-16 border-t border-[#f0f2f4] dark:border-gray-800">
              ...
              {TIMELINE.map((item, idx) => (
                <React.Fragment key={idx}>
                  <div className="flex flex-col items-center gap-1 pt-1">
                    <div className={`p-2 rounded-full ${item.isLatest ? 'bg-primary text-white shadow-lg shadow-blue-500/30' : 'bg-[#f0f2f4] dark:bg-gray-800 text-[#111318] dark:text-white'}`}>
                      <span className="material-symbols-outlined text-sm">{item.isLatest ? 'monitor_heart' : 'terminal'}</span>
                    </div>
                    {idx < TIMELINE.length - 1 && <div className="w-[2px] bg-[#f0f2f4] dark:bg-gray-800 h-full grow my-2"></div>}
                  </div>
                  <div className={`flex flex-1 flex-col ${idx < TIMELINE.length - 1 ? 'pb-8' : 'pt-1'}`}>
                    <div className="flex justify-between items-baseline mb-2">
                      ...
                      <span className={`text-xs font-mono px-2 py-1 rounded ${item.isLatest ? 'bg-primary text-white' : 'bg-primary/10 text-primary'}`}>{item.year}</span>
                    </div>
